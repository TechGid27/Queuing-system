...
<head>
    ...
    <title>{{ config('app.name', 'ACLC Queue System') }} — @yield('page-title', 'Home')</title>
...
<aside id="sidebar" class="fixed top-0 left-0 bottom-0 w-60 bg-slate-900 flex flex-col z-30 -translate-x-full lg:translate-x-0 transition-transform duration-200">
    ...
    <nav class="flex-1 px-3 py-4 overflow-y-auto">
        @if(auth()->user()->role === 'staff' || auth()->user()->role === 'admin')
            ...
            <hr class="mb-5">
            <a href="{{ auth()->user()->role === 'admin' ? route('admin.overview') : route('admin.queue', $departmentRouteParams) }}"
